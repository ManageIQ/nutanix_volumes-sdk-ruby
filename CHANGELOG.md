# Change Log
All notable changes to this project will be documented in this file.
This project adheres to [Semantic Versioning](http://semver.org/).

## [Unreleased]

## [0.2.3] - 2026-05-01
### Fixed
- Drop docs from published gem ([#24](https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/pull/24))

## [0.2.2] - 2025-11-25
### Fixed
- Fix extra files included in gem build ([#17](https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/pull/17))

## [0.2.1] - 2025-11-24
### Added
- Add a post-generate script to fix invalid constants ([#15](https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/pull/15))

### Fixed
- Fix `_UNKNOWN`/`_REDACTED` constants ([#16](https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/pull/16))

## [0.2.0] - 2025-11-24
### Added
- Add License ([#7](https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/pull/7))
- Add GitHub Action to autogenerate from the openapi spec ([#10](https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/pull/10))
- Add ability to one off ci jobs. ([#12](https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/pull/12))

### Updated
- Update actions/checkout action to v5 ([#9](https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/pull/9))
- Update nutanix_volumes gem ([#11](https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/pull/11))
- Update openapi generator to `v7.17.0` to use `api_model_base` ([#13](https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/pull/13))

## [0.1.0] - 2025-06-11
### Added
- Initial release

[Unreleased]: https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/compare/v0.2.3...HEAD
[0.2.3]: https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/compare/v0.2.3...v0.2.3
[0.2.2]: https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/compare/v0.2.1...v0.2.2
[0.2.1]: https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/compare/v0.2.0...v0.2.1
[0.2.0]: https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/ManageIQ/nutanix_volumes-sdk-ruby/tree/v0.1.0
